...
def make_function(code, defaults=None, lineno=0):
    ...
    args = [
        _ast.Name(id=argname, ctx=_ast.Param(), lineno=lineno, col_offset=0)
...
def make_function(code, defaults=None, annotations=(), kw_defaults=(), lineno=0):
    ...
    for argname in varnames:
        if argname in annotation_names:
            arg = [
                annotation for annotation in annotations if annotation.arg == argname
            ][0]
        else:
            arg = _ast.arg(
                annotation=None, arg=argname, lineno=lineno, col_offset=0
            )  # @UndefinedVariable
    ...
    if "return" in annotation_names:
        arg = [annotation for annotation in annotations if annotation.arg == "return"][
            0
        ]
...
class Instructions(CtrlFlowInstructions, SimpleInstructions):
    ...
    def decompile_block(self, ilst, stack_items=None, jump_map=False):
        return Instructions(
            ilst, stack_items=stack_items, jump_map=jump_map, outer_scope=self
        )
    ...
    def MAKE_FUNCTION(self, instr):
        ...
        if isinstance(doc, str):
            function.body.insert(
                0,
                _ast.Expr(
                    value=_ast.Str(s=doc, lineno=instr.lineno, col_offset=0),
                    lineno=instr.lineno,
                    col_offset=0,
                ),
            )
