...
def reduce_cmp(right):
    ...
    if isinstance(right, LogicalOp) and right.flag == "AND":
        ...
        if isinstance(right.right, _ast.Compare) and isinstance(
            right.parent, _ast.Compare
        ):
...
def JUMP_IF_X_OR_POP(operand):
    def JUMP_IF_Y_OR_POP(self, instr):
        ...
        if len(self.ilst) >= 2 and [ii.opname for ii in self.ilst[:2]] == [
            "ROT_TWO",
            "POP_TOP",
        ]:
            self.ilst.pop(0)
...
class CtrlFlowInstructions(object):
    ...
    def process_logic(self, logic_block):
        ...
        if logic_block[0].opname in JUMPS:
            ...
            if idx is None:
                if len(logic_block) == 3 and [ii.opname for ii in logic_block[1:]] == [
                    "ROT_TWO",
                    "POP_TOP",
                ]:
                    assert logic_block[0].opname in [
                        "JUMP_FORWARD",
                        "RETURN_VALUE",
                    ], logic_block
                    return None
                ...
                if len(parent) == 3 and [ii.opname for ii in parent[1:]] == [
                    "ROT_TWO",
                    "POP_TOP",
                ]:
